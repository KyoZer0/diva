...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl shadow-lg text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium mb-1">Factures Totales</p>
                    <h3 class="text-3xl font-bold">{{ $totalInvoices ?? 0 }}</h3>
                    <p class="text-purple-100 text-xs mt-2">
                        <span class="font-semibold">{{ isset($totalInvoiceAmount) ? number_format($totalInvoiceAmount, 0) : '0' }}</span> MAD
                    </p>
                </div>
                <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
